...
catchacc = ZonalStatistics(scatch, "Value", flowaccras, "MAXIMUM", ignore_nodata="DATA")
catchlakeacc = ZonalStatistics(scatch, "Value", lakeaccras, "MAXIMUM", ignore_nodata="DATA")
lakeind = Float(catchlakeacc)/Float(catchacc)
lakeind.save(wd+"lakeaccindex")
ZonalStatisticsAsTable(scatch, "Value", wd+"lakeaccindex","lakeaccindex", ignore_nodata="DATA", statistics_type="MAXIMUM")
...
catchacc = ZonalStatistics(scatch, "Value", flowaccras, "MAXIMUM", ignore_nodata="DATA")
catchresacc = ZonalStatistics(scatch, "Value", resaccras, "MAXIMUM", ignore_nodata="DATA")
resind = Float(catchresacc)/Float(catchacc)
resind.save(wd+"resaccindex")
ZonalStatisticsAsTable(scatch, "Value", wd+"resaccindex","resaccindex", ignore_nodata="DATA", statistics_type="MAXIMUM")
...
arcpy.Delete_management(catchacc)
arcpy.Delete_management(catchresacc)
arcpy.ClearEnvironment("extent")
...
lakindfields = ['OBJECTID','CatFlowAcc','CatArea','CatLakInd','CatResInd']
with arcpy.da.UpdateCursor(attriformat, lakindfields) as cursor:
    for row in cursor:
        print(row[0])
        for f in range(3,len(lakindfields)):
            if row[f]== None:
                print('pass')
            else:
                row[f]=row[1]*row[f]/row[2]
                cursor.updateRow(row)
del row
del cursor
...
arcpy.MakeFeatureLayer_management(slineattri, 'slineattrisub')
arcpy.SelectLayerByLocation_management('slineattrisub', 'INTERSECT', datadir+'sharepoint20180314\\gis\\hydro\\rufiji\\rufiji_subbasins.shp',selection_type='NEW_SELECTION')
slineattrirufi = outhydro+'streamnet118_attri_rufiji'
arcpy.CopyFeatures_management('slineattrisub', slineattrirufi)
#Create feature dataset
feature_dat = "rufijinet"
pr = arcpy.Describe(slineattri).SpatialReference
arcpy.CreateFeatureDataset_management(outhydro, feature_dat, pr)
arcpy.CopyFeatures_management(slineattrirufi,outhydro+"\\"+feature_dat+"/rufilines")
#Create geo network
out_name = "rufijinet"
in_source_feature_classes = "rufilines SIMPLE_EDGE NO"
arcpy.CreateGeometricNetwork_management(outhydro + "\\" + feature_dat, out_name, in_source_feature_classes)
networkrufi = outhydro + feature_dat + "\\" + out_name
#Set flow direction of geometric network, a pre-requisite for tracing
arcpy.SetFlowDirection_management(networkrufi, flow_option = "WITH_DIGITIZED_DIRECTION")

###################################################################
# Create points for tracing
arcpy.FeatureVerticesToPoints_management(in_features=slineattrirufi,out_feature_class=outhydro+'linemidpoints_attrirufi', point_location='MID')
...
array_sum =arcpy.da.FeatureClassToNumPyArray('spointlyrreduce', tabfields)
arcpy.da.NumPyArrayToTable(array_sum, scratch_tab)
arcpy.CreateTable_management(wd+'watershed_attri.gdb', 'RufiWs_Attri_Sum', scratch_tab)
...
memerror_tab = 'RufiWS_Attri_Sum_MemoryError'
...
arcpy.FindIdentical_management(out_tab,out_tab+'_identical',fields='GridID',output_record_option='ONLY_DUPLICATES')
dupliID = [id[0] for id in arcpy.da.SearchCursor(out_tab+'_identical', ['IN_FID'])]
duplicates = [[row[0],row[1],row[2]] for row in arcpy.da.SearchCursor(out_tab, ['OBJECTID','GridID','WsArea']) if row[0] in dupliID]
if len(duplicates)>0:
    d={}
    ldel=[]
...
with arcpy.da.UpdateCursor(rufiws_tab,watextdivlist) as cursor:
    for row in cursor:
        if row[0]>0:
            for i in range(1,len(watextdivlist)): #Iterate over every selected field after area and divide it by area
                row[i]=float(row[i])/float(row[0])
        cursor.updateRow(row)
del row
del cursor
with arcpy.da.UpdateCursor(rufiws_tab,areadivlist) as cursor:
    for row in cursor:
        for i in range(1,len(areadivlist)): #Iterate over every selected field after area and divide it by area
            row[i]=float(row[i])/float(row[0])
        cursor.updateRow(row)
del row
del cursor
...
for var in ['Dir','Geol','LC','Soil']:
    if 'Ws{}Maj'.format(var) not in [f.name for f in arcpy.ListFields(rufiws_tab)]:
        arcpy.AddField_management(in_table=rufiws_tab,field_name='Ws{}Maj'.format(var),field_type='TEXT')
    varfields = [f.name for f in arcpy.ListFields(rufiws_tab, '{}Sum*'.format(var))]
    varfields.insert(0, 'Ws{}Maj'.format(var))
    with arcpy.da.UpdateCursor(rufiws_tab, varfields) as cursor:
        for row in cursor:
...
#Using ArcHydroTools "Accumulate attributes", takes several seconds per stream
#Using arcpy.Statistics_Analysis takes over 25s per node.
"""
catstatslist = [[f.name, 'SUM'] for f in arcpy.ListFields('pointlyr') if not f.name in ['CatLakInd', 'CatResInd','CatElvMax','CatElvMin']][12:]  ####### UPDATE NUMBEER #########
for f in ['CatLakInd', 'CatResInd','CatElvMax']:
    catstatslist.append([f, 'MAX'])
catstatslist.append(['CatElvMin','MIN'])
catstatslist.append(['GridID','FIRST'])
catstatslist.append(['GridID','LAST'])

arcpy.Statistics_analysis('pointlyr', 'wsattri_raw', catstatslist)
#start=time.time()
with arcpy.da.SearchCursor('pointlyr', ["GridID"]) as cursor:
    for row in cursor:
        print(row[0])
        subcatch_id = row[0]
        expr = '"GridID" = %s' %subcatch_id
        arcpy.SelectLayerByAttribute_management('pointlyr', 'NEW_SELECTION', expr)
        start=time.time()
        arcpy.TraceGeometricNetwork_management(in_geometric_network= network, out_network_layer = "up_trace_lyr", in_flags = "pointlyr",
                                               in_trace_task_type= "TRACE_UPSTREAM")
        print("Tracing took "+str(time.time() - start))
        up_tr = "up_trace_lyr\\lines"
        memtab = r"in_memory\stats_table"
        start=time.time()
        arcpy.Statistics_analysis(up_tr, memtab, catstatslist)
        print("Statistics took "+str(time.time() - start))
        start=time.time()
        arcpy.Append_management(memtab, 'wsattri_raw', "TEST")
        print("Appending took "+str(time.time() - start))
        #Add try and excepts statements
#end = time.time()
#print(end - start)"""
...
##########################################################################################################################
############################################
## Identify all upstream catchments
#Test Geosyntec's propagator approach
#See https://github.com/Geosyntec/python-propagator/tree/07a351df05681cbd32ec1a5e3419b71dad9ad129/propagator

fields = [f.name for f in arcpy.ListFields(sline)]
fields = numpy.unique(fields)
array = arcpy.da.FeatureClassToNumPyArray(in_table=sline, field_names=fields)
array = array[['HydroID','NextDownID']]
...
def find_row_in_array(array, column, value):
    ...
    return row
def trace_upstream(subcatchment_array, subcatchment_ID, id_col='ID', ds_col='DS_ID', include_base=False, downstream=None):
    """
    ...
    if include_base:
        base_row = find_row_in_array(subcatchment_array, id_col, subcatchment_ID)
...
idlist = trace_upstream(subcatchment_array=array, subcatchment_ID=5166364, id_col='HydroID', ds_col='NextDownID', include_base= True, downstream=None)
...
def traceup_single(ranges):
    i, j = ranges[0], ranges[1]
    exprange='"OBJECTID">{0} AND "OBJECTID"<={1}'.format(i,j)
    if arcpy.Exists(outhydro+"\\test"):
        lyr = arcpy.management.MakeFeatureLayer(
                      outhydro+"\\test",
                      'layer{0}'.format(i), exprange)
        upcatchdic = defaultdict(list)
        with arcpy.da.SearchCursor(lyr, ["HydroID"]) as cursor:
            for row in cursor:
                #x=x+1
                #print(x)
                subcatch_id = row[0]
                #print(row[0])
                expr = '"HydroID" = %s' %subcatch_id
                arcpy.Exists
                arcpy.SelectLayerByAttribute_management(lyr, 'NEW_SELECTION', expr)
                if arcpy.Exists(network):
                    arcpy.TraceGeometricNetwork_management(in_geometric_network= network, out_network_layer = "up_trace_lyr", in_flags = lyr,
                                                           in_trace_task_type= "TRACE_UPSTREAM")
                    up_tr = "up_trace_lyr\\lines"
                    #Iterate through each reach upstream of gage
                    with arcpy.da.SearchCursor(up_tr, ["HydroID"]) as cursor_up:
                        for row_up in cursor_up:
                            upcatchdic[subcatch_id].append(row_up[0])
        outdic= upcatchdic
        return outdic
    # End
...
def trace_thread():
    ...
    result_dic = pool.map(traceup_single, ranges)

    # Concatenate the resulting arrays and create an output table
    # reporting any identical records.
    #result_dic = { k: v for d in result_dic for k, v in d.items() }
...
traceup_single([301,400])
trace_thread()"""

########################################################
#Depth to groundwater NOT SURE THE DATA IS OF GREAT VALUE
#GWdepth = datadir + 'McDonaldetal2012\\DepthToGroundwater\\xyzASCII_dtwmap_v1.txt'
#GWdepthras = wd + 'GWdepthras'
#arcpy.MakeXYEventLayer_management(table=GWdepth, in_x_field='X', in_y_field='Y', out_layer='DTW', spatial_reference = 4326)
#arcpy.CopyFeatures_management('DTW', wd+'GWdepth.shp')
#arcpy.PointToRaster_conversion(wd+'GWdepth.shp', 'DTWAFRICA_', GWdepthras, cellsize=0.05)
#arcpy.env.snapRaster = scatch
#cs = (arcpy.GetRasterProperties_management(scatch, 'CELLSIZEX')).getOutput(0)
#arcpy.Resample_management(wd+"BDTICM_M_250mclip.tif", depthrock3s, cs, resampling_type='BILINEAR')
#ZonalStatisticsAsTable(scatch, "Value", depthrock3s, "depthrock",ignore_nodata="DATA",statistics_type="MEAN")
...
##############################################################
# ATTEMPTS AT DOING TABULATE AREA ON NON-PROJECTED RASTER
""" RUNS OUT OF MEMORY
catch_origin = arcpy.Point(Raster(scatch).extent.XMin,Raster(scatch).extent.YMin)
tz_origin = arcpy.Point(Raster(dirtz).extent.XMin,Raster(dirtz).extent.YMin)
scatch_np = arcpy.RasterToNumPyArray(in_raster=Raster(scatch),lower_left_corner=catch_origin)
"""

"""NOT USEFUL AS REALLY DISTORTS RASTER TOO MUCH
sr = arcpy.SpatialReference(102022) #Africa Albers Equal Area Conic
dirtzproj = outhydro+"\\dirtzproj"
arcpy.ProjectRaster_management(dirtz, outhydro+"\\dirtzproj", sr, resampling_type='NEAREST')
scatchpolyproj = outhydro+"\\catchpolyproj"
arcpy.Project_management(scatchpoly, outhydro+"\\catchpolyproj", sr)
TabulateArea(scatch, "Value", dirtz, "Value", out_table="dirhisto")
"""