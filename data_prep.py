...
arcpy.SelectLayerByLocation_management('slineattrisub', 'INTERSECT', datadir+'sharepoint20180316\\gis\\hydro\\rufiji\\rufiji_subbasins.shp',selection_type='NEW_SELECTION')
...
sumout_tab = gdbname_ws+ 'RufiWs_Attri_Sum'
...
########################################################################################################################
#Create template table #Do not reiterate if re-running code and no copy has been made of data
maxout_tab = gdbname_ws+ 'RufiWs_Attri_Max'
scratch_tab = gdbname_ws+ 'scratch'
...
arcpy.Copy_management(in_data=sumout_tab,out_data=sumout_tab+'_20180324')
...
comparetabs=arcpy.TableCompare_management(sumout_tab, memerror_tab, sort_field='GridID', compare_type='SCHEMA_ONLY',continue_compare='CONTINUE_COMPARE',out_compare_file=wd+'compare')
#Append tables
...
arcpy.MakeTableView_management(sumout_tab, out_view='RufiWs_Attri_Sum_view', where_clause=expr)
...
arcpy.MakeTableView_management('RufiWs_Attri_Sum_nodupli', 'RufiWs_Attri_Sum_noduplilyr')
arcpy.AddJoin_management('RufiWs_Attri_Sum_noduplilyr', 'GridID', maxout_tab, 'GridID')
rufiws_tab=gdbname_ws+'RufiWs_Attri'
arcpy.CopyRows_management('RufiWs_Attri_Sum_noduplilyr',rufiws_tab)
arcpy.Delete_management('RufiWs_Attri_Sum_noduplilyr')
arcpy.DeleteField_management(rufiws_tab, ['OBJECTID_1','GridID_1'])

###################################################
# Compute final variables for watersheds
areadivlist = [f.name for f in arcpy.ListFields(rufiws_tab) if not f.name in ['OBJECTID','WsWatOcc','WsWatcha','WsWatSea','WsLen_1','GridID',
                                                                              'WsRoadDen','WsDamDen','WsMineDen','WsPAPer','WsLakInd', 'WsResInd','WsElvMax']] #Keep area as first field and don't divide for dam, roads, mines, and PA density
...
arcpy.CopyRows_management('slineruficatlyr',rufiorder1) #Does not seem to respect selection

#Format catchment attribute table into Ws table
appfields=[f.name for f in arcpy.ListFields(rufiorder1) if f.name not in ['Shape_Length','CatFlowAcc','CatElvMin','CatCurvAvg']][9:]
...
with arcpy.da.UpdateCursor(slineruficatws, ['CatFlowAcc','WsLakInd', 'WsResInd']) as cursor:
    for row in cursor:
        row[1]=row[1]/row[0]
        row[2]=row[2]/row[0]
        cursor.updateRow(row)
...
arcpy.AddJoin_management('slineruficatws_lyr', 'GridID', wd + gdbname_reach + '\\reach_attri','Value_')
slinerufifinal = finalgdb +'streamnet118_rufiji_final'
arcpy.CopyFeatures_management('slineruficatws_lyr', slinerufifinal)
arcpy.DeleteField_management(slinerufifinal, ['OBJECTID_1','Value_'])

#Compute reach slope
arcpy.AddField_management(slinerufifinal, field_name='ReaSloAvg',field_type='FLOAT')
arcpy.CalculateField_management(slinerufifinal, 'ReaSloAvg', "math.degrees(math.atan(float(!ReaElvMax!-!ReaElvMin!)/float(1000*!CatLen_1!)))", expression_type='PYTHON')
...
#Remaining tasks
    ...
    #Inspect OSM data to find additional reservoirs to add to the reservoir accumulation index calculation. e.g. Kihansi dam
    #Correct location of mines and dams
    #